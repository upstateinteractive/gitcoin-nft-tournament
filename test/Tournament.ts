...
describe("Tournament", function () {
  ...
  describe("endCurrentRound", function () {
    ...
    it("should revert if not called by the owner", async function () {
      await expect(
        tournament.connect(notOwner).endCurrentRound(1, C.roundResults1)
      )
        .to.be.revertedWith("Caller is not the owner")
    })
    it("should revert if the tournament has not started", async function () {
      await expect(
        tournament.connect(owner).endCurrentRound(2, C.roundResults1)
      )
    ...
    it("should revert if the tournament has reached the last round", async function () {
      ...
      await tournament.connect(owner).endCurrentRound(1, C.roundResults2)

      await expect(
        tournament.connect(owner).endCurrentRound(1, C.roundResults3)
      )
        .to.be.revertedWith("Tournament must not be reached the last round")
    })
  ...
  describe("endTournament", async function () {
    ...
    it("should revert if not called by the owner", async function () {
      await expect(
        tournament.connect(notOwner).endTournament(1, C.roundResults1)
      )
        .to.be.revertedWith("Caller is not the owner")
    })
    it("should revert if the tournament has not started", async function () {
      await expect(
        tournament.connect(owner).endTournament(2, C.roundResults1)
      )
    ...
    it("should correctly update balances", async () => {
      await tournament.connect(owner).endCurrentRound(1, C.roundResults1)
      await tournament.connect(owner).endCurrentRound(1, C.roundResults2)
      await tournament.connect(owner).endTournament(1, C.roundResults3)
